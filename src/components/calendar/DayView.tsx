...
export function DayView({ currentDate, events, onEventClick }: CalendarViewProps) {
  ...
  const formatTimeLabel = (hour: number) => {
    return formatTimeHour(new Date(new Date().setHours(hour, 0, 0, 0)));
  };
    ...
    <div className="flex flex-col flex-1 overflow-auto">
      <div className="grid grid-cols-2 border-b">
        ...
        <div className="p-2 text-center">
          <div className="font-medium">{formatDayAbbrev(currentDate)}</div>
        ...
        <div className="grid grid-cols-2 divide-y">
          {hours.map(hour => (
            <div key={hour} className="grid grid-cols-2 col-span-2 h-20">
              <div className="border-r p-2 text-xs text-gray-500">{formatTimeLabel(hour)}</div>
