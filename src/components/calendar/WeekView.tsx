...
import { formatTimeHour, formatDayAbbrev, formatDayNumber } from '@/utils/dateUtils';
  ...
  return (
    <div className="flex flex-col flex-1 overflow-auto">
      <div className="grid grid-cols-8 border-b">
        ...
        {/* Day headers */}
        {weekDays.map((day, i) => (
          <div key={i} className={cn('p-2 text-center border-r', isToday(day) ? 'bg-blue-50' : '')}>
            <div className="font-medium">{formatDayAbbrev(day)}</div>
            <div
              className={cn(
          ...
          {hours.map(hour => (
            <div key={hour} className="grid grid-cols-8 col-span-8 h-20">
              ...
              <div className="border-r p-2 text-xs text-gray-500">
                {formatTimeHour(new Date(new Date().setHours(hour, 0, 0, 0)))}
              </div>
              ...
              {weekDays.map((day, dayIndex) => {
                ...
                return (
                  <div
                    key={dayIndex}
                    className={cn('border-r h-full relative', isToday(day) ? 'bg-blue-50' : '')}
                  >
                    {dayEvents.map(event => {
                      const startHour = getHours(event.start);
                      if (startHour === hour) {
                          ...
                          >
                            <div className="p-1 overflow-hidden">
                              ...
                              <div className="truncate">
                                {formatTimeHour(event.start)} - {formatTimeHour(event.end)}
                              </div>
                              {event.assignedTo && (
